...
function request(handler, uri, baseURI, charset) {
  ...
  if (uri.spec !== response.uri) {
    // If `uri` was a string we create a nsIURI out of it. Please not that we
    // can not use `nsIIOService` as it uses protocol handler and we'll end
    // up with recursive loop.
    if ('string' === typeof uri) {
      let newURI = SimpleURI.createInstance(Ci.nsIURI)
      newURI.spec = uri
      uri = newURI
    }
  }
  ...
  // `response.uri`
    ...
    channel = IOService.newChannel(response.uri, null, charset || null)
    // If request was redirected we put save `originalURI` in the `channel`
    // and create a new `uri` for the URI to which request was redirected.
  ...
  return Object.create(response, { channel: { value: channel } })
}
...
const TProtocolHandler = Trait(
  ...
  Trait({
    ...
    newURI: function newURI(relativeURI, charset, baseURI) {
      // We need to create an `nsIURI` that will be returned by this method,
      // also please note that `nsIIOService` can't be used since it uses
      // protocol handler under neath and there for this protocol handler will
      // enter recursive loop.
      try {
      let response = request(this, relativeURI, baseURI, charset)
      // We need to store response so that in `newChannel` will be able to
      // use it without a call to an protocol handler.
      let responses = this.responses || (this.responses = {})
      responses[response.uri] = response
      return response.channel.URI
      } catch (e) {
        console.exception(e)
        throw e
      }
    ...
    newChannel: function newChannel(uri) {
      ...
      // If request was a redirect we copy privileges
      if (this.scheme !== uri.scheme)
        response.channel.owner = securityManager.getCodebasePrincipal(uri)
